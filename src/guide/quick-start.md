...
Ruso scans a target against a library of ready-made scripts. No scripts to write,
...
scripts — in one command:

```bash
ruso scan --family web --target https://target.example.com
```

Ruso pulls every published `web` script from the registry, runs them against your
...
## 3. Search for a specific script
...
| Verdict | Meaning |
|---------|---------|
| **detected** | The script matched — a finding was emitted (with evidence). |
| **not detected** | The target didn't meet the script's conditions. |
...
- **Want to write your own script?** [Write Your Own Script](../rsl/first-check.md)
...
- **Want to share scripts?** See [Publishing & Installing](../registry/publishing.md).
